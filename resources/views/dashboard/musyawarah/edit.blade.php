...
<div class="col-lg-8">
    ...
    {{-- method mempengaruhi route post akan mengarah ke store --}}
      <form method="POST" action="/dashboard/musyawarah/{{ $post->slug }}" class="mb-5"
        enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="mb-3">
          <label for="title" class="form-label">Nama Musyawarah</label>
        ...
        <div class="mb-3">
          <label for="time" class="form-label">Tanggal & Waktu Musyawarah</label>
          <input type="datetime-local" class="form-control @error('time')is-invalid @enderror" id="time" name="time" required autofocus value="{{ old('time', $post->time) }}">
          @error('time')
                <div class="invalid-feedback">
                  {{ $message }}
                </div>
        ...
        <div class="mb-3">
          <label for="location" class="form-label">Lokasi Musyawarah</label>
        ...
        <div class="mb-3">
          <label for="body" class="form-label">Agenda Musyawarah</label>

          @error('agenda')
              <p class="text-danger">{{ $message }}</p>
          @enderror
            <input id="agenda" type="hidden" name="agenda" value="{{ old('agenda', $post->agenda) }}">
            <trix-editor input="agenda"></trix-editor>
        </div>

        {{-- Editor body menggunakan trix --}}
        <div class="mb-3">
          <label for="body" class="form-label">Hasil Musyawarah</label>

          @error('hasil')
              <p class="text-danger">{{ $message }}</p>
          @enderror
            <input id="hasil" type="hidden" name="hasil" value="{{ old('hasil', $post->hasil) }}">
